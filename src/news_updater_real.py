...
class NewsUpdaterReal:
    ...
    def load_news_api(self, keyword):
        with open(f'../newsapi/{keyword}.json', "r") as f:
            # with open(f'../newsapi/kak.json', "r") as f:
            return json.load(f)

    # def filter_articles(self, articles):
    #     return [r for r in articles if 'https://removed.com' not in r['url'] and r['source']['name'] is not None and r['title'] is not None and r['description'] is not None and r['url'] is not None and r['urlToImage'] is not None and r['publishedAt'] is not None and r['content'] is not None]
