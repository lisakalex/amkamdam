...
def paged(read_file, file):
    ...
    newsapi1 = json.loads(requests.get(url).text)
    # with open('test/newsapi.json', "r") as f:
    #     newsapi = json.load(f)

    with open('article.html', 'r') as file1:
        soup = BeautifulSoup(file1.read(), features="html.parser")
    for i in newsapi1['articles']:
...
def main():
    ...
    #
    # boxing = []
    # with open('html/paged/boxing-1.json') as file:
    #     try:
    #         boxing = json.load(file)
    #     except Exception as e:
    #         print(e)
    #
    #     pass
    ...
    #
    # with open('html/paged/boxing-1.json', 'w') as f:
    #     json.dump(boxing, f)
